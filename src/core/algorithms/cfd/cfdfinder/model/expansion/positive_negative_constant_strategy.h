...
private:
    void ProcessForId(Entries& buffer_entries, size_t replaced_index, Frontier& frontier,
                      Row const& inverted_pli_rhs, PruningStrategy& pruning_strategy, size_t id,
                      std::vector<int>&& valid_constants, Cover const& cover) const override;
    std::vector<int> FilterForSupport(std::vector<int>&& valid_constants,
                                      ...
                                      Cover const& cover) const override;
